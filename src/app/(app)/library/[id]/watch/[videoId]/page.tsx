...
import {
    ChevronDown, ChevronUp, Play, File, MessageSquare, ThumbsUp, Send, Download, Menu, X,
    ArrowLeft, AudioLines, CheckCircle2
} from "lucide-react";
...
export default function CourseVideoPage() {
    ...
    const [expandedSectionIndex, setExpandedSectionIndex] = useState(() => {
        // Set the initial expanded section based on the currently playing video
        const activeVideoIndex = courseData.curriculum.findIndex(section =>
            section.lessons.some(lesson => lesson.id === params.videoId)
        );
        return activeVideoIndex !== -1 ? activeVideoIndex : 0;
    });

    const [activeVideoId, setActiveVideoId] = useState(params.videoId);

    // Function to handle section toggling
    const toggleSection = (index: any) => {
        setExpandedSectionIndex(prevIndex => (prevIndex === index ? [prevIndex, index] : index));
    };

    // Function to handle video selection
    const handleVideoClick = (sectionIndex: any, lessonId: any) => {
        setActiveVideoId(lessonId);
        setExpandedSectionIndex(sectionIndex); // Open the new section
    };
        ...
        <main className="">
            <div className="max-w-[1600px] mx-auto">
                ...
                <div className="lg:grid lg:grid-cols-4 min-h-screen">
                    ...
                    <div className={`
            fixed inset-0 lg:relative bg-white transform transition-transform duration-300 ease-in-out md:max-w-[370px]
          ...
          `}>
                    {/* <div
                        className={`
        fixed inset-0 lg:relative bg-white transform transition-transform duration-300 ease-in-out max-w-[350px]
                                    ...
                                    {courseData.curriculum.map((section, sectionIndex) => (
                                        <div key={section.title} className="bg-white">
                                            <button
                                                ...
                                                className={`w-full px-4 py-3 flex items-center justify-between hover:bg-gray-50 ${expandedSectionIndex === sectionIndex ? "bg-gray-50" : ""}`}
                                                ...
                                                <div className="">
                                                    {section.lessons.map((lesson) => (
                                                        <Link
                                                            ...
                                                            onClick={() => handleVideoClick(sectionIndex, lesson.id)}
                                                            className={`flex items-center gap-3 px-4 py-3 hover:bg-gray-100 ${lesson.id === activeVideoId ? "bg-appPurple-900/10" : ""}`}
                                                        >
                                                            <div className="flex-shrink-0">
                                                                {lesson.type === "video" ? (
                                                                    lesson.id === activeVideoId ?
                                                                        <AudioLines className={`w-4 h-4 text-appPurple-900`} />
                                                                        :
                                                            ...
                                                            <div className="flex-1 min-w-0">
                                                                <p className={`text-sm font-medium truncate whitespace-break-spaces ${lesson.id === activeVideoId ? "text-appPurple-900" : "text-gray-900"
                                                                    }`}>
                                                                    {lesson.title}
                                                                </p>
                                                            ...
                                                            {lesson.completed && (
                                                                <div className="flex-shrink-0">
                                                                    <CheckCircle2 className={`w-4 h-4 rounded-full ${lesson.id === activeVideoId ? "text-appPurple-900/70" : "text-gray-400"}`} />
                                                                    {/* <div className={`w-2 h-2 rounded-full ${lesson.id === activeVideoId ? "bg-green-500" : "bg-gray-300"}`}>
                    ...
                    <div className="lg:col-span-3 py-20">
                        ...
                        <div className="p-4 lg:p-6">
                            <div className="max-w-7xl mx-auto space-y-8">
                                {/* Video Info */}
                                <div>
                                    <h1 className="text-2xl font-medium tracking-[1.2] text-gray-900 mb-2">
                                    ...
                                    <p className="text-gray-600">
                                        ...
                                        <span onClick={() => setExpandedSections(!expandedSections)} className="text-appPurple-900 text-sm hover:text-appPurple-900/80 hover:transition-colors cursor-pointer"> 
                                            {expandedSections ? " Read Less..." : " Read More..."}</span>
                                    </p>

                                    {
                                        expandedSections && <ContentSection database={courseData.content ?? []} />
                                    }
